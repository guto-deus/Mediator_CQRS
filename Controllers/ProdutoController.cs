...
namespace Mediator_CQRS.Controllers
{
    ...
    public class ProdutoController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Post(CriarProdutoComando command)
        {
        ...
        [HttpPut]
        public async Task<IActionResult> Put(AtualizarProdutoComando command)
        {
            var response = await _mediator.Send(command);
            return Ok(response);
        }
        ...
        {
            var obj = new DeletarProdutoComando (id);
